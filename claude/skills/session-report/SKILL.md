...
Follow [`session-report-format.md`](../../rules/session-report-format.md). **`Slice:` must be a machine id** (`CM3`, `CDRIVE-7`, `W18`) so the chain can advance — except planning PRs (`Slice: FM PRD planning`).

## Assurance paragraph

Same voice as [`ceo-communication.md`](../../rules/ceo-communication.md) — sacrifice grammar, outcome first:
...
- Type explicit OK in chat/session (DNS, schema)
- Add credential in Cursor Secrets / Claude Code environment settings
...
Incident tone: [`references/ralph-incidents.md`](./references/ralph-incidents.md)
